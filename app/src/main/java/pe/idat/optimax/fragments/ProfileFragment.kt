            ...
            activity?.runOnUiThread {
                if (call.isSuccessful) {
                    ...
                    if (client != null) {
                        ...
                        mBinding.ietCod.setText(client.cod_client.toString())
                        mBinding.ietCod.isEnabled = false
    ...
    private fun updateClient() {
        ...
        mBinding.btnUpdate.setOnClickListener {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                var cod = mBinding.ietCod.text!!.trim().toString().toInt()
                ...
                //var district = mBinding.ietDistrict.text?.trim().toString()

                var district = DistrictResponse(codDistrict = 1, name = "District")

                var client = ClientDto(cod_client = cod,
                    name = name,
                    pSurname = pSurname,
                    mSurname = mSurname,
                    DNI = DNI,
                ...
                val call = getRetrofit().create(APIService::class.java).putUpdateClient(client)
                activity?.runOnUiThread {
                    if (call.isSuccessful) {
                        getGson()
